...
$.extend(Datepicker.prototype, {
	...
	clear: function(target) {
		...
		if (inst) {
			...
			if (inst.get('selectDefaultDate') && inst.get('defaultDate')) {
			    ...
			    this.setDate(target, []); // ADDED 11/12/2012
	...
	selectDate: function(target, elem) {
        if (target && $(target).attr('readonly')) return; // ADDED 12/12/55 FOR SUPPORT READONLY MODE
		...
		if (inst && !this.isDisabled(target)) {
			...
			if (multiSelect) {
				...
				//------------ ADDED , ADD NEW AND REMOVE OLD ONE
				else if (!found && inst.selectedDates.length  >= multiSelect) {
                    inst.selectedDates.splice(0, 1);
                    inst.selectedDates.push(date);
                }//------------ END ADDED
			}
			else if (rangeSelect) {
