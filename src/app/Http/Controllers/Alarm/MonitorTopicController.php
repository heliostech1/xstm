...
class MonitorTopicController extends MyBaseController
{
    public $criteriaPrefix = "monitorTopic";
    ...
    public function index(Request $request)
    {
        ...
        return $this->openView('monitorTopic.listMonitorTopic', $this->data);
    }
    ...
    function getDataTable(Request $request) {
        $output = MonitorTopic::getDataTable($request, $this->getCriteriaDatas($request));
    ...
    function _getDataForViewEdit($request) {
        ...
        $result = (!empty($paramId))? MonitorTopic::getData($paramId): null;
    ...
    function add(Request $request)
    {
        ...
        return $this->openView('monitorTopic.addMonitorTopic', $this->data);
    ...
    function addSubmit(Request $request) {
        ...
        if (!$validator->fails())
        {
            ...
            if ( MonitorTopic::addData($inputDatas)) {     
                $request->session()->flash('message', "เพิ่มข้อมูลใหม่แล้ว");
                return redirect("monitorTopic/index?keep=1");
            }
        }
    
        $this->data['message'] = $this->getResponseMessage($request, $validator, MonitorTopic::getErrors());
        $this->_collectPostData($request);
        $this->data['pageMode'] = 'add';
    ...
    function editSubmit(Request $request) {
        ...
        {            
            ...
            if ( MonitorTopic::editData($mongoId, $inputDatas)) {
    ...
    function delete(Request $request)
    {
        ...
        $paramId = $request->input($this->criteriaPrefix.'_tableSelectedId');
        $object = MonitorTopic::getData($paramId);
        
        if (!$object) {
            $request->session()->flash('message', "ไม่พบรหัสข้อมูล '".$paramId."'");
        }
        else {
            if (MonitorTopic::deleteData($paramId)) {
                $request->session()->flash('message', "ลบข้อมูล '".$object['name']."' แล้ว");
            }
            else {
                $request->session()->flash('message', MonitorTopic::errors());
            }
        }
