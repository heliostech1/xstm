...
class VCareTypeController extends MyBaseController
{
    public $criteriaPrefix = "vCareType";
    ...
    function getDataTable(Request $request) {
        $output = VCareType::getDataTable($request, $this->getCriteriaDatas($request));
    ...
    function _getDataForViewEdit($request) {
        ...
        $result = (!empty($paramId))? VCareType::getData($paramId): null;
    ...
    function addSubmit(Request $request) {
        ...
        if (!$validator->fails())
        {
            ...
            if ( VCareType::addData($inputDatas)) {     
                $request->session()->flash('message', "เพิ่มข้อมูลใหม่แล้ว");
                return redirect("vCareType/index?keep=1");
            }
        }
    
        $this->data['message'] = $this->getResponseMessage($request, $validator, VCareType::getErrors());
        $this->_collectPostData($request);
        $this->data['pageMode'] = 'add';
    ...
    function editSubmit(Request $request) {
        ...
        {            
            ...
            if ( VCareType::editData($mongoId, $inputDatas)) {
    ...
    function delete(Request $request)
    {
        ...
        $paramId = $request->input($this->criteriaPrefix.'_tableSelectedId');
        $object = VCareType::getData($paramId);
        
        if (!$object) {
            $request->session()->flash('message', "ไม่พบรหัสข้อมูล '".$paramId."'");
        }
        else {
            if (VCareType::deleteData($paramId)) {
                $request->session()->flash('message', "ลบข้อมูล '".$object['name']."' แล้ว");
            }
            else {
                $request->session()->flash('message', VCareType::errors());
            }
        }
