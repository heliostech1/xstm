...
use App\Http\Models\Common\Refrigerant;

class RefrigerantController extends MyBaseController
{
    public $criteriaPrefix = "refrigerant";
    ...
    function getDataTable(Request $request) {
        $output = Refrigerant::getDataTable($request, $this->getCriteriaDatas($request));
    ...
    function _getDataForViewEdit($request) {
        ...
        $result = (!empty($paramId))? Refrigerant::getData($paramId): null;
    ...
    function addSubmit(Request $request) {
        ...
        if (!$validator->fails())
        {
            ...
            if ( Refrigerant::addData($inputDatas)) {     
                $request->session()->flash('message', "เพิ่มข้อมูลใหม่แล้ว");
                return redirect("refrigerant/index?keep=1");
            }
        }
    
        $this->data['message'] = $this->getResponseMessage($request, $validator, Refrigerant::getErrors());
        $this->_collectPostData($request);
        $this->data['pageMode'] = 'add';
    ...
    function editSubmit(Request $request) {
        ...
        {            
            ...
            if ( Refrigerant::editData($mongoId, $inputDatas)) {
    ...
    function delete(Request $request)
    {
        ...
        $paramId = $request->input($this->criteriaPrefix.'_tableSelectedId');
        $object = Refrigerant::getData($paramId);
        
        if (!$object) {
            $request->session()->flash('message', "ไม่พบรหัสข้อมูล '".$paramId."'");
        }
        else {
            if (Refrigerant::deleteData($paramId)) {
                $request->session()->flash('message', "ลบข้อมูล '".$object['name']."' แล้ว");
            }
            else {
                $request->session()->flash('message', Refrigerant::errors());
            }
        }
