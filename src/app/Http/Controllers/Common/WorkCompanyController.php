...
class WorkCompanyController extends MyBaseController
{
    public $criteriaPrefix = "workCompany";
    ...
    function getDataTable(Request $request) {
        $output = WorkCompany::getDataTable($request, $this->getCriteriaDatas($request));
    ...
    function _getDataForViewEdit($request) {
        ...
        $result = (!empty($paramId))? WorkCompany::getData($paramId): null;
    ...
    function addSubmit(Request $request) {
        ...
        if (!$validator->fails())
        {
            ...
            if ( WorkCompany::addData($inputDatas)) {     
                $request->session()->flash('message', "เพิ่มข้อมูลใหม่แล้ว");
                return redirect("workCompany/index?keep=1");
            }
        }
    
        $this->data['message'] = $this->getResponseMessage($request, $validator, WorkCompany::getErrors());
        $this->_collectPostData($request);
        $this->data['pageMode'] = 'add';
    ...
    function editSubmit(Request $request) {
        ...
        {            
            ...
            if ( WorkCompany::editData($mongoId, $inputDatas)) {
    ...
    function delete(Request $request)
    {
        ...
        $paramId = $request->input($this->criteriaPrefix.'_tableSelectedId');
        $object = WorkCompany::getData($paramId);
        
        if (!$object) {
            $request->session()->flash('message', "ไม่พบรหัสข้อมูล '".$paramId."'");
        }
        else {
            if (WorkCompany::deleteData($paramId)) {
                $request->session()->flash('message', "ลบข้อมูล '".$object['name']."' แล้ว");
            }
            else {
                $request->session()->flash('message', WorkCompany::errors());
            }
        }
