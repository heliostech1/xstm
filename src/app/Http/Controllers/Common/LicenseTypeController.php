...
use App\Http\Models\Common\LicenseType;

class LicenseTypeController extends MyBaseController
{
    public $criteriaPrefix = "licenseType";
    ...
    function getDataTable(Request $request) {
        $output = LicenseType::getDataTable($request, $this->getCriteriaDatas($request));
    ...
    function _getDataForViewEdit($request) {
        ...
        $result = (!empty($paramId))? LicenseType::getData($paramId): null;
    ...
    function addSubmit(Request $request) {
        ...
        if (!$validator->fails())
        {
            ...
            if ( LicenseType::addData($inputDatas)) {     
                $request->session()->flash('message', "เพิ่มข้อมูลใหม่แล้ว");
                return redirect("licenseType/index?keep=1");
            }
        }
    
        $this->data['message'] = $this->getResponseMessage($request, $validator, LicenseType::getErrors());
        $this->_collectPostData($request);
        $this->data['pageMode'] = 'add';
    ...
    function editSubmit(Request $request) {
        ...
        {            
            ...
            if ( LicenseType::editData($mongoId, $inputDatas)) {
    ...
    function delete(Request $request)
    {
        ...
        $paramId = $request->input($this->criteriaPrefix.'_tableSelectedId');
        $object = LicenseType::getData($paramId);
        
        if (!$object) {
            $request->session()->flash('message', "ไม่พบรหัสข้อมูล '".$paramId."'");
        }
        else {
            if (LicenseType::deleteData($paramId)) {
                $request->session()->flash('message', "ลบข้อมูล '".$object['name']."' แล้ว");
            }
            else {
                $request->session()->flash('message', LicenseType::errors());
            }
        }
